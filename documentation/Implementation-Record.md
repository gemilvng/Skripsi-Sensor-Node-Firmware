...
- Superseded after Phase 3 close (recorded here so the closed-phase notes above are not silently contradicted by later code): the fixed 4-byte `"PING"` payload (`MESH_HELLO_PAYLOAD`) was replaced by a per-node self-identifying payload `"Data from 0x%04x"` (`MESH_PAYLOAD_FORMAT`, e.g. `"Data from 0x49cc"`), formatted once at init from each board's own address so the manager can distinguish originators in payload captures; 26 B on air, still under the 64 B packet cap. The Phase 3 transmit power of 6 dBm was later raised to 17 dBm in `config.h` for outdoor range, and was lowered to 3 dBm at the bench/rooftop when forcing nodes close together (the 2026-06-15 rooftop run was at 3 dBm); the committed `config.h` value is the source of truth for any given build.
...
Status — **parked (deliberate). Firmware complete; criteria #1 and #2 met via the logical-isolation backup; physical relay and criterion #3 deferred to the Phase 8 field campaign.**
...
- Cause: a single interior wall added only ≈ 15 dB; isolating the A–C link (driving it below the −123 dBm sensitivity floor) needs ≈ 42 dB more. The link-budget extrapolation in the report predicts 90–200 m rooftop hops close with ≥ 31 dB margin, so the phase is expected to pass at outdoor inter-building scale where distance and building obstruction provide the isolation indoor walls could not. Phase 4 remains open until a run demonstrates `hops > 1` with relay through B. The firmware was committed at `58246b0` (manager-addressed TX selection and the `mesh_route` link-metric dump, alongside the packet-size cap); this is the firmware commit, not a phase-closure commit — the `Phase 4 closes at commit ...` line is deferred until the relay demonstration passes.
- Second hardware attempt (2026-06-15, rooftop, base firmware without logical isolation, all three nodes at 3 dBm): manager `0x2510` at Point 5, relay `0x7ADC` at Point 6, far node `0x8020` at Point 3 (see `dev-resource/Deployment_Point_Options.kml`), logging from the manager. Both ordinary nodes still reached the manager directly — `hops=1`, single hop — so rooftop geometry at these points did not drive the far link below the sensitivity floor either. Criterion #2 fails again, this time outdoors. Log: `logs/device-monitor-260615-100811.log`. This is the failure mode the logical-isolation backup exists for.
- Backup method exercised (2026-06-16): with the logical-isolation flag the far node is *forced* off the direct link, and criteria #1 and #2 are demonstrated end-to-end (see the subsection below). This validates the routing/forwarding logic but not the physical RF multi-hop benefit, so it is recorded as a clearly-labelled backup result, distinct from any physical-isolation result.
- Criterion #3 (power relay B off → A loses route to C; B on → route restored) has not been captured in either method yet, so the phase is not closed under either route. The cheapest close is a single relay-kill/restore capture logged from the far node.
- Decision (2026-06-16): Phase 4 is **deliberately parked**, not closed, and work proceeds to Phase 5. Rationale: the firmware is complete and the routing/forwarding logic is already validated by the logical-isolation backup, so the only outstanding items (physical `hops>1` and criterion #3) are field/RF-logistics problems, not code problems. Phase 5 (sensor bring-up) has no dependency on multi-hop — a sampling stub behaves identically at one hop or three — so starting it does not build on unproven ground. The physical multi-hop demonstration and the relay-kill/restore capture are carried forward to the **Phase 8 five-node field campaign**, whose done-criteria already mandate a forced multi-hop layout and a kill-a-relay test; folding Phase 4's remaining proof into that deployment avoids a separate field trip. This is a conscious deviation from the strict one-phase-at-a-time discipline (principle #2), taken because the blocker is geography, not firmware; it must not become a silent permanent skip — if the available rooftop points cannot produce isolation, that geometry problem is to be solved before Phase 8, not discovered during it.

### Phase 4 — Logical isolation (backup method)

The backup for forcing multi-hop when physical separation fails: firmware makes chosen nodes drop each other's beacons/route-tables, simulating "out of range" even when the radio link is fine, so the mesh is forced to route through an intermediate node.
It proves the **routing/forwarding logic**, not the **physical RF multi-hop benefit**, and must be reported as a backup, separate from any physical result — an examiner could otherwise read firmware-forced isolation as assuming the conclusion.

- What it does: a node ignores `SYNC_BEACON`/`ROUTE_TABLE` from denied peers (joins and data are allowed, so the handshake still completes), forcing it to learn the manager only through a relay's forwarded beacon. Validated end-to-end on 2026-06-16: far node `0x8020` has no direct route to the manager, reaches `0x2510` at `hops=2 valid=1` via a relay, and the manager receives DATA from both far nodes (`mesh_rx src=0x8020` + `src=0x236c`). Logs: far node `logs/device-monitor-260616-051006.log`, manager `logs/device-monitor-260616-054601.log`.
- Where it lives (intentionally kept out of the tracked tree): the denylist header `include/mesh_isolation.h`, the reapply script `scripts/apply_isolation_patch.py`, and four `#ifdef MESH_FORCE_ISOLATION`-gated edits in the vendored `.pio/libdeps/ttgo-t-beam/LoRaMesher/src/protocols/lora_mesh/services/network_service.cpp`. The switch is `-DMESH_FORCE_ISOLATION` in `platformio.ini`, commented out by default. Full design and implementation record: `dev-resource/logical-isolation-feature.md`.
- Carve-out to principle #5 (no vendoring): this feature deliberately edits the vendored library, the one place the accept/drop decision is reachable (the public API exposes no hook below `ProcessReceivedMessage`). The edits are all gated by the build flag, so the default build compiles as if they do not exist; the vendored tree is wiped on any `pio` reinstall and reapplied by the script. This is the sole, conscious exception to principle #5, scoped to the isolation demo only.
...
Implementation:
- The sensor source chosen is the ESP32's own internal die-temperature sensor (not an external sensor or a synthetic stub), reported as the node's chip temperature. Confirmed present on the board's `ESP32-D0WDQ6-V3` (original ESP32 silicon); it measures the uncalibrated silicon die temperature, not ambient — useful for per-node trends and thermal-stress, reads above ambient because of self-heating, and carries a large per-chip absolute error. This is the `chip_temp` source for the Phase 6 telemetry payload (`dev-resource/packet-structure.md`).
- New module `src/sensor.cpp` + `src/sensor.h` follows the established module shape (free functions, module-private `static` state, wrapper header includes no third-party headers, per-module `TAG`): `bool init_sensor()` takes and logs one boot reading (`sensor_init_ok first_temp_c=..`), and `sensor_sample_task` samples on the fixed cadence.
- API decision: the reading is taken via the Arduino core's `temperatureRead()` (declared in `esp32-hal.h`, reached through `<Arduino.h>`). On the original ESP32 target the core defines it as `(temprature_sens_read() - 32) / 1.8` in `esp32-hal-misc.c` (verified in the installed framework `framework-arduinoespressif32` 2.0.17), returning °C; it needs no init, no `temp_sensor_start/stop`, and no `extern "C"` declaration. This deliberately deviates from `dev-resource/packet-structure.md` §3/§6, which names the ESP-IDF `temperature_sensor` driver (`temperature_sensor_get_celsius`) — that driver is compiled only for the S2/S3/C-series targets and does not link for `ESP32-D0WDQ6-V3`. The deviation is recorded in a code comment in `sensor.cpp`; correcting §3/§6 of the spec is a Phase 6 follow-up.
- Cadence is `SENSOR_SAMPLE_INTERVAL_MS = 30000` (new `constexpr` in `config.h`). 30 s was chosen because: the die signal moves over tens of seconds and the reading is coarse (int8 / ±several °C), so faster sampling adds no information; >60 s would miss thermal trends; and 30 s aligns with both the ~33 s mesh superframe (so the value is at most one send-period stale when it feeds the Phase 6 packet) and the existing `MESH_RX_REPORT_INTERVAL_MS` heartbeat, keeping the log on one rhythm.
- Task priority is derived from the existing tasks rather than chosen independently: `SENSOR_SAMPLE_TASK_PRIORITY` is defined **by reference** as `= MESH_APP_TASK_PRIORITY` (2) and `SENSOR_SAMPLE_TASK_STACK = MESH_APP_TASK_STACK` (4096), so the derivation is explicit in code. `sensor_sample_task` is housekeeping of the same class as the mesh app tasks: equal priority to its peers and below every LoRaMesher internal task (3/14/15) so the mesh always preempts it. Equal priority is safe because the task does one register read plus one log line then sleeps 30 s, so it is runnable for microseconds per interval and cannot starve its peers. The `xTaskCreate` lives in `setup()` (`main.cpp`) with the other tasks, gated on `init_sensor()` returning true (Architecture.md §1).
- The task is a clean single periodic trigger via `vTaskDelayUntil` (drift-free), so unlike `mesh_rx_task` it needs no mixed-trigger justification comment. Each tick emits one data-bearing line `sensor_sample temp_c=%.2f count=%u uptime_ms=%llu`, maintains the `sensor_sample_count` counter named in Architecture.md §4, and sources its timestamp from `esp_timer_get_time()`. `temp_c` is logged as a float in Phase 5; rounding to the `int8` packet field is deferred to the Phase 6 pack.
- Phase 5 scope is sample + log only. The packet integration (replacing the `"Data from 0x%04x"` string payload with the 7-byte little-endian struct, the `seq` counter, the int8 rounding, and reconciling *where* the temperature is sampled — the dedicated-task model in Architecture.md vs the sample-at-send model in `packet-structure.md` §3/§6, which currently disagree) is deliberately Phase 6 work and was not started here.
- Hardware verification (2026-06-16, two boards, same binary): manager `0x2510` and ordinary node `0x236c`. Both done-criteria met. Cadence is exact — `uptime_ms` steps 1036 → 31037 → 61037 … in 30 s increments — and `count` is contiguous with stable formatting (criterion #1). The mesh ran concurrently and was not stalled: the manager's `mesh_rx_stats` heartbeat fired on its own 30 s cadence interleaved with the sensor samples, and it received DATA from `0x236c` (`mesh_rx src=0x236c bytes=4461...3663` = "Data from 0x236c") (criterion #2). Logs: `logs/device-monitor-260616-164221.log` and `logs/device-monitor-260616-165515.log` (`0x2510`), `logs/device-monitor-260616-164755.log` (`0x236c`).
- Cross-node temperature finding: the two boards read ~44 °C apart — `0x2510` ≈ 84–88 °C, `0x236c` ≈ 39–43 °C — across the run. This was established as a **fixed per-chip offset, not a real temperature difference**: the boot reading (taken ~1 s after power-on, before the die can self-heat by tens of degrees) already shows the full ~44 °C gap with both dies essentially at ambient; the gap is reproducible across separate reboots; the hotter board is the manager, which transmits *less* than the node (TX destination, silent on TX), so it is not workload; and a Celsius-vs-Fahrenheit unit mismatch is excluded numerically (no unit conversion maps one reading onto the other). The original ESP32 sensor has no published accuracy/offset spec, so no documented tolerance is claimed — only that the difference is offset, expressed in raw counts (~185 vs ~103). Implication: per-node trends are trustworthy; cross-node absolute comparison is not. A one-time per-node offset calibration (measure each board cold at a common ambient, store an offset constant) is deferred as optional polish, not a Phase 5 blocker.
- Phase 5 closes at commit `d44fbee`.
...
Implementation:
- The payload is the combined telemetry + routing-table packet documented in `dev-resource/packet-structure.md` and `dev-resource/routing-table-reporting.md`: a fixed 8-byte header (`seq` uint16, `timestamp` uint32 ms-since-boot, `chip_temp` int8, `entry_count` uint8) followed by `entry_count` six-byte routing entries (`destination`, `next_hop`, `hop_count`, and a `flags` byte with bit0 = valid, bit1 = is_network_manager), packed little-endian. The "node address" the done-criteria call for is delivered out-of-band as the LoRaMesher receive callback's `source` argument rather than repeated in the payload, so it costs no airtime.
- Scope note: Phase 6's minimal requirement is address + seq + reading, but the packet was deliberately extended in this phase to also carry each node's own routing table, so the monitoring server can reconstruct every node's distance-vector view (the per-node next-hop / hop-count the sink cannot derive locally). This is a conscious expansion beyond the minimum, taken because the routing entries ride free inside the data slot already sized for a 64-byte frame (`setMaxPacketSize(64)`), so a separate packet and a second slot are avoided. The full design and the honest scope of what the topology view does and does not demonstrate are in `dev-resource/routing-table-reporting.md`.
- New module `src/telemetry.cpp` + `src/telemetry.h` is the single definition of the wire byte order: `telemetry_pack_header` / `telemetry_pack_route_entry` build the payload and `telemetry_parse_header` / `telemetry_parse_route_entry` read it back. Fields are read and written by hand (not a struct memcpy) so the format does not depend on compiler layout/padding and ports to the separate sink parser. Parsing is zero-copy — the receiver reads fields straight out of the received buffer with no allocation and no intermediate copy.
- Encode (`mesh_tx_task`, `src/mesh.cpp`): builds the packet at send time from a `static uint32_t` seq counter (low 16 bits on the wire, incremented only after a successful `Send()` so a retried slot reuses the same seq and the receiver sees no gap), a send-time `esp_timer_get_time()/1000` timestamp, the latest published chip-temperature (`sensor_latest_chip_temp_c()`), and the routing table from `GetRoutingTable()` (already fetched for destination selection) — every non-self route, capped at `TELEMETRY_MAX_ROUTE_ENTRIES` (7) to keep the frame within the 64-byte cap. The payload is packed into a stack buffer and assigned into a pre-sized module-static vector so `Send()` adds no per-iteration heap.
- Sizing: payload = 8 + 6 × entry_count bytes; with the 10-byte DATA header and the 64-byte frame cap, 54 payload bytes are available, so at most 7 entries fit. For the five-node deployment a node reports up to four destinations → ≤ 32 B payload / 42 B on air, ~22 B of headroom.
- Decode (`mesh_rx_task`, on the manager, which is the data destination): parses the header in place, emits one `mesh_rx src=.. seq=.. ts_ms=.. temp_c=.. routes=N` summary line, then one `mesh_route_report src=.. dst=.. next=.. hops=.. valid=.. mgr=..` line per routing entry, so the manager's serial log shows every node's routing table. A frame that fails validation (wrong length / not telemetry) falls back to a `mesh_rx_malformed` hex dump so decode errors are visible rather than silently dropped.
- The Phase 3/5 self-identifying string payload (`MESH_PAYLOAD_FORMAT = "Data from 0x%04x"`) is removed: the constant in `config.h` and the string-building block in `init_mesh()` are gone, replaced by the encoded packet.
- Hardware verification (2026-06-17, three boards, manager `0x2510` logging; senders `0x49cc` and `0x8020`; log `logs/device-monitor-260617-075807.log`): over a ~9-minute run the manager received `seq` 0–16 from `0x49cc` (17 packets) and `seq` 0–14 from `0x8020` (15 packets), both contiguous with no gaps and zero `mesh_rx_malformed` — 100 % delivery, byte-identical decode, and monotonic per-source seq, so both done-criteria are met.
- Distance-vector behaviour observed in the same run (a bonus beyond the done-criteria, read from the `mesh_route_report` lines): the topology was not a static star. For the first ~4 minutes both ordinary nodes reached the manager directly at 1 hop but reached *each other* only at 2 hops via the manager (`next=0x2510 hops=2`); the direct `0x49cc`↔`0x8020` link was then learned and each collapsed that route to 1-hop direct, asymmetrically (`0x49cc` at its seq 5, `0x8020` at its seq 3); and at `0x8020` seq 3 the node transiently lost its direct route to the manager and rerouted to `0x2510` via the relay `0x49cc` at `hops=2`, recovering to 1 hop on the next send. These are genuine distance-vector route-selection and reconvergence events, but transient (a discovery warm-up and a brief link flap), not a sustained physical multi-hop path.
- Latency note: no packet was lost or reordered, but `0x8020`'s telemetry developed a steady ~one-superframe (~32 s) delivery lag after its first two packets (with one ~two-superframe spike on the seq-3 reroute), while `0x49cc` was delivered promptly in-slot; this is delivery jitter, not loss.
- Forward path: the closed packet is a verified baseline. Future metrics the sink can compute from the `seq` / `timestamp` stream and LoRaMesher APIs (PDR, latency, link quality, hop count — `packet-structure.md` §4) need no firmware change and belong to Phase 7; metrics that require new node-measured payload fields are an explicit, re-verified format revision, with `seq` / `timestamp` kept at fixed front offsets so the sink's PDR/latency math survives the growth.
- Phase 6 closes at commit `<recorded after the phase-boundary commit>`.
